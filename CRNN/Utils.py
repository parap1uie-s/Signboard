...
def DataGen(datapath, height, width, batch_size=32, phase='train'):
    ...
    while True:
        ...
        for row in choiced_data.iterrows():
            r = row[1]
            Img = Image.open(os.path.join(datapath, phase, r['classid'], r['filepath'])).resize((width,height),Image.ANTIALIAS)
